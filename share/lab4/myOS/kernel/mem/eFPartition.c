...
void eFPartitionWalkByAddr(unsigned long efpHandler){
	...
	*/
	...
	showeFPartition((struct eFPartition*)efpHandler);
	// print EEB
	EEB* eeb = (EEB*)((struct eFPartition*)efpHandler)->firstFree;
	while(eeb != 0){
		showEEB(eeb);
...
unsigned long eFPartitionTotalSize(unsigned long perSize, unsigned long n){
	...
	*/
	...
	unsigned long size = perSize * n;
	if(size % 8 != 0){
		size = (size / 8 + 1) * 8;
	}
	size += n * EEB_size;
	size += eFPartition_size;
	return size;
...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n){
	...
	*/
	...
	EEB* eeb = (EEB*)(start + eFPartition_size);
	for(int i = 0; i < n - 1; i++){
		eeb->next_start = start + eFPartition_size + (i + 1) * (perSize + EEB_size);
		eeb = (EEB*)eeb->next_start;
	}
	eeb->next_start = 0;
